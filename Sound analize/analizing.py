...
for i in range(1, len(tmpData)//furieCount):
    ...
    sptr2 = sptr1
wav = wave.open("spectrEquals_"+filename, mode="wb")
...
speach2=[]
for i in range(len(tmpData)//furieCount):
    sptr = FFTAnalysis(tmpData[i * furieCount:(i + 1) * furieCount])
    if(hasMainTone(sptr)):
        for j in range(i*furieCount,(i+1)*furieCount):
            speach2.append(tmpData[j])
wav = wave.open("spectrTones_"+filename, mode="wb")
wav.setparams((1,signal.getSampwidth(),signal.getFramerate(), len(speach)//signal.getSampwidth(),"NONE","not compressed"))
wav.writeframes(byteArrFromPoints(speach2,signal.getSampwidth()))
wav.close()
